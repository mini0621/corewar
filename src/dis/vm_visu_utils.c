...
/*   Updated: 2019/07/02 10:02:13 by mndhlovu         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */

#include "corewar.h"

void			dis_init_visu(t_dis_game *game, t_visu *visu, int pl_num)
{
	initscr();
	cbreak();
	noecho();
...
static void		dis_restart_all(t_dis_game *game, int p_num)
{
	dis_end_visu(game->visu);
	dis_init_visu(game, game->visu, p_num);
}

static int		dis_output_util(t_dis_game *game, int *flag, int *p_num)
{
	ft_bzero(&(game->dismemdump[0]), CHAMP_MAX_SIZE * sizeof(t_ucc));
	*p_num = dis_load_player(game);
	if (*p_num == -1)
	{
		*flag = 0;
		dis_end_visu(game->visu);
