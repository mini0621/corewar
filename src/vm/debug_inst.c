...
/*   Updated: 2019/06/28 10:27:19 by mndhlovu         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
				...
				, t_process *caller, int val)
{
	...
	if (opcode == e_st && inst->args[1].type == e_reg)
		l = ft_asprintf(&tmp, "st r%hhu('%08x') -> r%hhu!\n",
				inst->args[0].value.u_reg_val, val
				, inst->args[1].value.u_reg_val);
	else if (opcode == e_st)
		l = ft_asprintf(&tmp, "st r%hhu('%08x') -> memdump(%i)!\n",
				inst->args[0].value.u_reg_val
				, val, inst->args[1].value.u_dir_val);
...
static int		deb_ld(t_process *caller, t_inst *inst
				, char **tmp, int val)
{
	t_opcode	opcode;
	...
	if ((opcode = (get_op(inst))->opcode)
		...
		return (deb_ld_util(caller, inst, tmp, val));
	return (0);
}
